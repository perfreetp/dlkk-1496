              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {notifications.map(n => (
                  <div key={n.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-slate-50 text-xs">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-slate-700">{getRecipientName(n.recipientId)}</p>
                      <p className="text-slate-500">{n.channel === 'SMS' ? '短信渠道' : '站内消息'} · {formatTime(n.sentAt)}</p>
                    </div>
                    <span className={clsx(
                      'badge',
                      n.status === 'SENT' ? 'badge-green' : n.status === 'DELIVERED' ? 'badge-green' : 'badge-red'
                    )}>
                      {n.status === 'SENT' ? '已发送' : n.status === 'DELIVERED' ? '已送达' : '失败'}
                    </span>
                  </div>
                ))}
              </div>
